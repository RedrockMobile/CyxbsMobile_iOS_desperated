...
- (void)getCollageData{
    ...
    for (NSString *name in [self.nameList objectForKey:@"name"]) {
        ...
        [manager GET:@"http://118.24.175.82/search/school/1" parameters:@{@"name":name} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        [manager GET:@"http://118.24.175.82/search/school/2" parameters:@{@"name":name} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            subjects = [NSDictionary dictionaryWithDictionary:responseObject];
            //            dispatch_semaphore_signal(semaphore);
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            //            NSLog(@"%@---",error);
            //            dispatch_semaphore_signal(semaphore);
