...
//                [self layerHeight] - (coord.y * _mapTileSize.height + _mapTileSize.height / 2));
    ...
    int height = [self layerHeight]/_mapTileSize.height;
...
-(void)moveTileFromCoord:(CGPoint)fromCoord toCoord:(CGPoint)toCoord
{
    uint32_t gid = [self.layerInfo tileGidAtCoord:fromCoord];
    
    if( gid )
    {
        int fz = fromCoord.x + fromCoord.y * self.layerInfo.layerGridSize.width;
        int tz = toCoord.x + toCoord.y * self.layerInfo.layerGridSize.width;
        
        // remove tile from GID map
        
        
        SKSpriteNode* tileNode = (SKSpriteNode*)[self childNodeWithName:[NSString stringWithFormat:@"*/%d", fz]];
        if(tileNode) {
            self.layerInfo.tiles[tz] = self.layerInfo.tiles[fz];
            self.layerInfo.tiles[fz] = 0;
            
            SKAction *action = [SKAction moveToX:[self pointForCoord:toCoord].x*32+16
                                        duration:0.2];
            [tileNode runAction:action];
        }
    }
}
...
{
	// create the map
	JSTileMap* map = [[JSTileMap alloc] initWithMapNamed:mapName withBaseZPosition:baseZPosition andZOrderModifier:zOrderModifier];
    return map;
}

- (id)initWithMapNamed:(NSString*)mapName {
    return [self initWithMapNamed:mapName withBaseZPosition:0.0f andZOrderModifier:-20.0f];
}

- (id)initWithMapNamed:(NSString*)mapName withBaseZPosition:(CGFloat)baseZPosition andZOrderModifier:(CGFloat)zOrderModifier {
    self = [self init];
    if (self) {
        ...
        // set zPosition range
        if (baseZPosition < (baseZPosition + (zOrderModifier * (self.zOrderCount + 1))))
        {
            self->_minZPositioning = baseZPosition;
            self->_maxZPositioning = baseZPosition + (zOrderModifier * (self.zOrderCount + 1));
        }
        else
        {
            self->_maxZPositioning = baseZPosition;
            self->_minZPositioning = baseZPosition + (zOrderModifier * (self.zOrderCount + 1));
        }
        ...
        for( TMXLayerInfo *layerInfo in self.layers )
        {
            ...
            {
                TMXLayer *child = [TMXLayer layerWithTilesetInfo:self.tilesets layerInfo:layerInfo mapInfo:self];
                child.zPosition = baseZPosition + ((self.zOrderCount - layerInfo.zOrderCount) * zOrderModifier);
#ifdef DEBUG
        ...
        // add tile objects
        for (TMXObjectGroup* objectGroup in self.objectGroups)
        {
#ifdef DEBUG
            NSLog(@"Object Group %@ has zPosition %f", objectGroup.groupName, (baseZPosition + (self.zOrderCount - objectGroup.zOrderCount) * zOrderModifier));
#endif
            ...
            for (NSDictionary* obj in objectGroup.objects)
            {
                ...
                if (num && [num intValue])
                {
                    TMXTilesetInfo* tileset = [self tilesetInfoForGid:[num intValue]];
                    if (tileset)	// add a tile object if it is apropriate.
                    {
                        ...
                        if (self.orientation == OrientationStyle_Isometric)
                        {
                            ...
                            CGPoint coords = [self screenCoordToPosition:CGPointMake(x, y)];
                            pt = CGPointMake((self.tileSize.width / 2.0) * (self.tileSize.width + coords.x - coords.y - 1),
                                             (self.tileSize.height / 2.0) * (((self.tileSize.height * 2) - coords.x - coords.y) - 2));
                        ...
                        {
                            pt = CGPointMake(x + (self.tileSize.width / 2.0), y + (self.tileSize.height / 2.0));
                        }
                        SKTexture* texture = [tileset textureForGid:[num intValue] - tileset.firstGid + 1];
                        SKSpriteNode* sprite = [SKSpriteNode spriteNodeWithTexture:texture];
                        sprite.position = pt;
                        sprite.zPosition = baseZPosition + ((self.zOrderCount - objectGroup.zOrderCount) * zOrderModifier);
        ...
        // add image layers
        for (TMXImageLayer* imageLayer in self.imageLayers)
        {
            ...
            image.zPosition = baseZPosition + ((self.zOrderCount - imageLayer.zOrderCount) * zOrderModifier);
            [self addChild:image];
#ifdef DEBUG
