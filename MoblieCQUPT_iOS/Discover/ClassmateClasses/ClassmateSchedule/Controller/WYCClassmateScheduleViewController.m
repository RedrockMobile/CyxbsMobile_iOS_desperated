//
//  WYCClassmateScheduleViewController.m
//  MoblieCQUPT_iOS
//
...
#import "DateModle.h"
#import "WYCClassmateScheduleView.h"
#import "WYCClassmateScheduleDetailView.h"
#import "WYCClassmateScheduleShowDetailView.h"
...
@interface WYCClassmateScheduleViewController ()<UIScrollViewDelegate, WYCClassmateScheduleViewDelegate>
...
@property (nonatomic, strong) WYCClassmateScheduleDataModel *model;
...
@implementation WYCClassmateScheduleViewController
...
-(instancetype)initWithClassmateNum:(NSString *)classmateNum{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(ModelDataLoadSuccess)
                                                 name:@"WYCClassmateScheduleDataModelDataLoadSuccess" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(ModelDataLoadFailure)
                                                 name:@"WYCClassmateScheduleDataModelDataLoadFailure" object:nil];
...
- (void)initModel{
    ...
    if (!self.model) {
        ...
        self.model = [[WYCClassmateScheduleDataModel alloc]init];
        [self.model getClassBookArrayFromNet:self.stuNum];
    }else{
...
- (void)ModelDataLoadSuccess{
    ...
    @autoreleasepool {
        for (int dateNum = 0; dateNum < self.dateModel.dateArray.count + 1; dateNum++) {
            ...
            WYCClassmateScheduleView *view = [[WYCClassmateScheduleView alloc]initWithFrame:CGRectMake(dateNum*_scrollView.frame.size.width, 0, _scrollView.frame.size.width, _scrollView.frame.size.height)];
...
- (void)initWeekChooseBar{
    self.weekChooseBar = [[WYCWeekChooseBar alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 39*autoSizeScaleY) nowWeek:self.dateModel.nowWeek];
...
-(void)updateScrollViewFame{
    ...
    if (self.hiddenWeekChooseBar) {
        ...
        for (int i = 0; i < 26; i++) {
            //NSLog(@"num:%d",i);
            WYCClassmateScheduleView *view = _scrollView.subviews[i];
...
- (void)showDetail:(NSArray *)array{
    ...
    //往全屏view上添加内容
    WYCClassmateScheduleShowDetailView *detailClassBookView  = [[WYCClassmateScheduleShowDetailView alloc]initWithFrame:CGRectMake(0, 2 * SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT)];
