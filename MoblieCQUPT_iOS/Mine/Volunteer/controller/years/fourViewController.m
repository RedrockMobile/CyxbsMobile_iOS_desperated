...
+ (void)initialize{
    if (self == [fourViewController class]){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 ){
        ...
        for (int i = 0; i<array.count; i++) {
            QueryModel *modelPer = self.mutableArray[3][i];
    ...
    QueryModel *model = self.mutableArray[3][indexPath.row];
    ...
    if (indexPath.row == 0) {
        ...
        cell.yearsLabel.text = [NSString stringWithFormat:@"%ld",year-indexPath.section-1];
