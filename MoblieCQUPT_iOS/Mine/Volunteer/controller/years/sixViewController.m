...
@interface sixViewController ()<UITableViewDataSource,UITableViewDelegate>
...
+ (void)initialize{
    if (self == [sixViewController class]){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 ){
        ...
        for (int i = 0; i<array.count; i++) {
            QueryModel *modelPer = self.mutableArray[1][i];
    ...
    QueryModel *model = self.mutableArray[1][indexPath.row];
    ...
    if (indexPath.row == 0) {
        ...
        cell.yearsLabel.text = [NSString stringWithFormat:@"%ld",year+1-indexPath.section];
