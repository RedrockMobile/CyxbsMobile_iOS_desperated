...
+ (void)initialize{
    if (self == [fiveViewController class]){
...
- (void)noRecord{
    ...
    self.noRecordImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, MAIN_SCREEN_W, MAIN_SCREEN_H-64-39)];
...
- (void)viewDidLoad {
    ...
    self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0,MAIN_SCREEN_W, MAIN_SCREEN_H-64) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 ){
        ...
        for (int i = 0; i<array.count; i++) {
            QueryModel *modelPer = self.mutableArray[2][i];
    ...
    QueryModel *model = self.mutableArray[2][indexPath.row];
    ...
    if (indexPath.row == 0) {
        ...
        cell.yearsLabel.text = [NSString stringWithFormat:@"%ld",year-indexPath.section];
